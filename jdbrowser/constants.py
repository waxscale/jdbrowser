# TokyoNight Dark theme colors
HIGHLIGHT_COLOR = '#44475a'
HOVER_COLOR = '#5c6370'
SLATE_COLOR = '#2e303e'
# Dark placeholder thumbnail background and lighter font color for readability
PLACEHOLDER_COLOR = '#1c1c1c'
PLACEHOLDER_TEXT_COLOR = '#a0a0a0'
TEXT_COLOR = '#c0caf5'
BACKGROUND_COLOR = '#2e303e'
BORDER_COLOR = '#44475a'
TAG_COLOR = '#3d59a1'
BUTTON_COLOR = '#d7ba7d'
DELETE_BUTTON_COLOR = '#f7768e'
BREADCRUMB_BG_COLOR = 'transparent'
BREADCRUMB_ACTIVE_COLOR = TEXT_COLOR
BREADCRUMB_INACTIVE_COLOR = MUTED_COLOR

# Additional TokyoNight palette values for enhanced styling
BG_COLOR = '#1a1b26'
SURFACE_COLOR = '#24283b'
SURFACE2_COLOR = '#1f2335'
MUTED_COLOR = '#a9b1d6'
ACCENT_COLOR = '#7dcfff'
